...
def ticker_historical_data(ib, tickerPackage: Ticker, startDate, endDate, barSizeSetting,
    ...
    barSizeSetting = me.bars_size(barSizeSetting)
    if not endDate: endDateDT = datetime.now(startDate.tzinfo)
    else: endDateDT = endDate
    return ib.reqHistoricalData(contract, endDateTime=endDate, durationStr=me.duration(endDateDT-startDate, **kwargs),
...
def ticker_historical_data_trades_populate_db(ib, tickers, barSizes, startDate, endDate, uploader, useRTH=None):
    ...
    for barsSize in barSizes:
        ...
        for ticker in tickers:
            ...
            last = df['date'].iloc[-1]
            widths.iloc[-1] = min(me.to_timezone(datetime.now(), me.get_timezone(last)) - last , barSizeTD)
