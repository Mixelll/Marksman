...
def join_indexed_tables(conn, tableNames, columns, index, join=None,  schema=None, startDate=None, endDate=None):
    ...
    comp += composed_from_join(tables=zip(schema, tableNames), using=[index]*len(tableNames), join=join)
...
def build_data(conn_or_engine, tickers, barSizesSamples, inputColumns, targetColumns,
    ...
    if order == 'C':
        inputColumnsBar = ['.'.join(e[1:3]) + str(e[0]) for x in barSizes for e in product(reversed(range(0,barSizesSamples[x])), table_names(x), inputColumns)]
    elif order == 'F':
        inputColumnsBar = ['.'.join(e[0:2]) + str(e[2]) for x in barSizes for e in product(table_names(x), inputColumns, reversed(range(0,barSizesSamples[x])))]
